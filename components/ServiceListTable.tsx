...
const ServiceListTable: React.FC<ServiceListTableProps> = ({ id, data, title, nameMapping }) => {
  ...
  const apiKey = "AIzaSyBk1LqHcluUWth13I2ezcgvUJcLRD9Rjrc"; // API key injected at runtime

  // Check columns globally for the view
  ...
  // Calculate dynamic column widths (approximately)
  // Total units: Service (3) + Coord (2) + Team (3) + Standby (2) = 10 (if all present)
  // Using percentages for HTML table
  const getColWidths = () => {
    const weights = [
  ...
  const callGemini = async (promptType: 'announce' | 'analyze') => {
    ...
    if (promptType === 'announce') {
        ...
        prompt = `Analyze the following service roster JSON for insights.
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
                ...
                {!isLoading && (
                    <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-2">
                        ...
                        <button 
                            onClick={() => setShowModal(false)}
                            className="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-slate-50 transition-colors"
                        >
                            Close
                        </button>
                    </div>
      ...
      <div id={id} className="bg-white p-6 max-w-full text-slate-900">
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {Object.entries(data)
              .sort(([dateA], [dateB]) => {
                 // Sort Unspecified Date to the top
