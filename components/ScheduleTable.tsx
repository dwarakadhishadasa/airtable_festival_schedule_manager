...
import { HEADER_IMAGE_URL, getHeaderImageBase64 } from '../services/pdfService';
...
const ScheduleTable: React.FC<ScheduleTableProps> = ({ id, data, title }) => {
  const [headerImage, setHeaderImage] = useState<string>(HEADER_IMAGE_URL);

  useEffect(() => {
    // Attempt to get the cached base64 version for display consistency
    const loadCachedImage = async () => {
        const cached = await getHeaderImageBase64();
        if (cached) {
            setHeaderImage(cached);
        }
    };
    loadCachedImage();
  }, []);

  // Check columns globally
  ...
  // Activity (4), Location (3), Timings (3)
  const getColWidths = () => {
    const weights = [
    ...
    <div className="bg-white">
      ...
      <div id={id} className="bg-white p-6 max-w-full text-slate-900">
        ...
        <div className="mb-6 w-full">
            <img src={headerImage} alt="Schedule Header" className="w-full h-auto" />
        </div>
